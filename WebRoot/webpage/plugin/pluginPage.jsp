  ...
  <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="div" action="templateController.do?savePlugin" >
		...
		<fieldset class="step">
			<div class="form">
		      ...
		      <input class="inputxt" id="name" name="name" ignore="false" value="${pluginPage.name }" datatype="*2-50">
		      <span class="Validform_checktip">插件名称围在2~50位字符</span>
		    </div>
		    <div class="form">
		      <label class="Validform_label">类型:</label>
		        <input type="radio"  name="type" value="1" <c:if test="${pluginPage.type == 1 }">checked="checked"</c:if>> &nbsp;即插即用型
		        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	     		<input type="radio"  name="type" value="2" <c:if test="${pluginPage.type == 2 }">checked="checked"</c:if>> &nbsp;用户定制
		    </div>
		    <div class="form">
		       <label class="Validform_label">开发者:</label>
		       <input name="userId" type="hidden" value="${pluginPage.userId }" id="userId">
	           <input name="nickName" class="inputxt" value="${nickName }" id="nickName" readonly="readonly"  />
		    ...
		    <div class="form">
				...
				<input class="inputxt" id="coverPath_a" name="icon" value="${pluginPage.icon }" ignore="ignore" readonly="readonly" hidden="true"/>
				<span class="Validform_checktip"></span><span id="picsuccess"></span>
				<div id="previewImg" style="margin-top:15px"><a href="${server }/${pluginPage.icon}"><img id="preImg" style="width:90px;height:90px;background-color:#ccc;border:1px solid #333" src="${server }/${pluginPage.icon}"/></a></div>
			   ...
			   <span class="Validform_checktip">点击图片可以查看原图</span> <span
				id="filediv"></span>
		    </div>
			<div class="form">
		      <label class="Validform_label">插件描述:</label>
		      <input class="inputxt" id="description" name="description" ignore="false" style="width: 500px"  datatype="*10-250" value="${pluginPage.description }">
		      <span class="Validform_checktip" style="float: right;">插件描述在2~250位字符</span>
		    </div>
		    <div class="form">
		      <label class="Validform_label">成本价:</label>￥
		      <input class="inputxt" id="price" name="price" ignore="false"  datatype="n,float" value="${price }">/月
		      <span class="Validform_checktip"></span>
		    </div>
		    <div class="form">
		      <label class="Validform_label">有效期:</label>
		      <input class="inputxt" id="valid" name="valid" ignore="false" datatype="n" value="${pluginPage.valid }">月
		      <span class="Validform_checktip">填写整数</span>
		    </div>
		    <div class="form">
		      <label class="Validform_label">使用说明:</label>  
		      <textarea rows="6" class="inputxt" style="width:80%" name="guide" id="guide">${pluginPage.guide }</textarea>
		      <span class="Validform_checktip"></span>
		    </div>
		    <div class="form">
