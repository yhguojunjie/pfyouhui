...
<html>
 ...
 <body style="overflow-y: hidden" scroll="no">
  ...
  <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="div" action="brandController.do?saveBrand">
		...
		<fieldset class="step">
			<div class="form">
		      ...
		      <input class="inputxt" id="seq" name="seq" ignore="false"
					 value="${brandPage.seq}" datatype="n">
		      <span class="Validform_checktip"></span>
		    </div>
			<div class="form">
			<c:if test="${brandPage.nickName==null}">
		      <label class="Validform_label">用户id:</label>
		      <input class="inputxt" id="userId"  readonly="readonly"  name="userId" ignore="false"
					 value="${brandPage.userId}" datatype="n">
			 <t:choose hiddenName="userId" hiddenid="id" url="brandController.do?userList" name="userList" icon="icon-choose" title="用户列表" textname="userId,nickName" isclear="true"></t:choose>
		      <span class="Validform_checktip"></span>
		     </c:if>
		     <c:if test="${brandPage.nickName!=null}">
		      <label class="Validform_label">用户id:</label>
			...
			<div class="form">
		      ...
		      <input class="inputxt" readonly="readonly" id="nickName" name="nickName" ignore="false"
					 value="${brandPage.nickName}">
		      <span class="Validform_checktip"></span> 
		    </div>
		    <div class="form">
				<label class="Validform_label">品牌logo:</label> 
				<input class="inputxt" id="wxLogo_a" name="logo" value="${brandPage.logo }" ignore="false" datatype="*" readonly="readonly" hidden="true"/> 
				<span class="Validform_checktip">插件图标路径不能为空</span>
				<div id="wxLogo_previewImg" style="margin-top:15px">
				<a href="${server }/${brandPage.logo}"><img style="width:200px;height:90px;background-color:#ccc;border:1px solid #333" src="${server }/${brandPage.logo}"/></a>
				</div>
		    ...
		    <div class="form">
		     ...
		     <c:if test="${brandPage.hideState==null}">   
		     ...
		     <c:if test="${brandPage.hideState=='0'}">   
			  <input   id="need" ignore="false" name="hideState" type="radio" value="0"  Checked />显示
			  &nbsp; &nbsp; &nbsp; &nbsp;
              <input   id="need" ignore="false" name="hideState" type="radio" value="1"  />隐藏
             </c:if>
             <c:if test="${brandPage.hideState=='1'}">   
