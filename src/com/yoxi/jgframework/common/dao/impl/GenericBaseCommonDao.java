...
import com.yoxi.jgframework.annotation.JWhatEntityTitle;
import com.yoxi.jgframework.common.dao.IGenericBaseCommonDao;
import com.yoxi.jgframework.common.dao.jdbc.JdbcDao;
import com.yoxi.jgframework.common.exception.BusinessException;
import com.yoxi.jgframework.common.hibernate.qbc.CriteriaQuery;
import com.yoxi.jgframework.common.hibernate.qbc.DetachedCriteriaUtil;
import com.yoxi.jgframework.common.hibernate.qbc.HqlQuery;
import com.yoxi.jgframework.common.hibernate.qbc.PageList;
import com.yoxi.jgframework.common.hibernate.qbc.PagerUtil;
...
import com.yoxi.jgframework.util.ToEntityUtil;
...
public abstract class GenericBaseCommonDao<T, PK extends Serializable> implements IGenericBaseCommonDao {
	...
	public Session openSession() {
		// openSession()打开的session与上下文无关,必须手动关闭
	...
	public List<DBTable> getAllDbTableName() {
		...
		for (String key : (Set<String>) metaMap.keySet()) {
			...
			try {
				c = Class.forName(key);
				JWhatEntityTitle t = c.getAnnotation(JWhatEntityTitle.class);
	...
	public int getCount(Class<T> clazz) {
		int count = DataAccessUtils.intResult(getSession().createQuery("select count(*) from " + clazz.getName())
				.list());
		return count;
	}
...
//		 queryListObjectBySQL(sql, paramsMap, clazz, scalarMap)
	...
	public PageList getPageListBySql(final HqlQuery hqlQuery, final boolean isToEntity) {
		...
		List list = null;
		if (isToEntity) {
			list = ToEntityUtil.toEntityList(query.list(), hqlQuery.getClass1(), hqlQuery.getDataGrid().getField()
					.split(","));
		} else {
			list = query.list();
		}
	...
	public List<Map<String, Object>> findForJdbc(String sql, int page, int rows) {
		// 封装分页SQL
		sql = JdbcDao.jwhatCreatePageSql(sql, page, rows);
