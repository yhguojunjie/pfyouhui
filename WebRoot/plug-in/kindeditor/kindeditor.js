...
var _VERSION = '4.1.1 (2012-06-10)-1',
...
var _INLINE_TAG_MAP = _toMap('a,abbr,acronym,b,basefont,bdo,big,br,button,cite,code,del,dfn,em,font,i,img,input,ins,kbd,label,map,q,s,samp,select,small,span,strike,strong,sub,sup,textarea,tt,u,var'),
	_BLOCK_TAG_MAP = _toMap('address,applet,blockquote,body,center,dd,dir,div,dl,dt,fieldset,form,frameset,h1,h2,h3,h4,h5,h6,head,hr,html,ins,isindex,li,map,menu,meta,noframes,noscript,object,ol,p,pre,script,style,table,tbody,td,tfoot,th,thead,title,tr,ul'),
...
function _mediaEmbed(attrs) {
	...
	if(html5tag=="iframe"){
		...
		html += '  webkit-playsinline></'+html5tag+'>';
	}
	return html;
}
...
KEditor.prototype = {
	...
	text : function(val) {
		var self = this;
		if (val === undefined) {
			return _trim(self.html().replace(/<(?!img|embed|audio|video|iframe).*?>/ig, '').replace(/&nbsp;/ig, ' '));
		} else {
	...
	count : function(mode) {
		...
		if (mode === 'text') {
			return self.text().replace(/<(?:img|embed|audio|video|iframe).*?>/ig, 'K').replace(/\r\n|\n|\r/g, '').length;
		}
		return 0;
	},
	exec : function(key) {
...
_plugin('core', function(K) {
	...
	self.beforeSetHtml(function(html) {
		return html.replace(/<(embed|audio|video|iframe)[^>]*>(?:<\/(embed|audio|video|iframe)>)?/ig, function(full) {
